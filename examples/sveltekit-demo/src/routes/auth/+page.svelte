<script>
	...
	onMount(() => {
		...
		const hasLoggedInBefore = localStorage.getItem('easyauth_demo_user');
	...
	async function autoLogin() {
		try {
			signingIn = true;
			// Small delay to show "Signing you in..." message
			await new Promise(resolve => setTimeout(resolve, 800));
			initializeDemoSession();
			goto('/dashboard');
		} catch (error) {
			info(error instanceof Error ? error.message : 'Auto sign-in failed.');
			signingIn = false;
			showLoginOptions = true;
		}
	}
	...
	async function handleGoogleLogin() {
		try {
			...
			localStorage.setItem('easyauth_demo_user', 'true');
			initializeDemoSession();
			goto('/dashboard');
		} catch (error) {
...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4">
	<div class="max-w-md w-full">
		...
		<div class="bg-white rounded-2xl shadow-xl p-8">
			...
			{#if isReturningUser && signingIn}
				<!-- Auto-login UI for returning users -->
